...
class PoseEstimation:
    ...
    def mv_Calculator(self,pre_kp_new, cur_kp_new):
        ...
        for p_kp, c_kp in zip(pre_kp_new, cur_kp_new):
            ...
            # magnitude_dxdy.append(c_kp_y-p_kp_y)
        ...
        for i, ang in enumerate(angles):
            ...
            list_dy_sum[idx] += magnitude_dxdy[i]
        ...
        return max_dy, max_degree

    def euler2dist(self, dy, max_degree):
        ...
        # self.pose2ROS(self.IMU_pitch)

        df = pd.DataFrame([{'1.IMU_pitch(deg)': round(math.degrees(self.IMU_pitch),4), '2.IMU_diff (deg)': round(math.degrees(self.IMU_pitch_diff),4), '3.Max Degree(deg)': max_degree, '4.Mag dy(px)': dy, '5.Modeling dy(rad)': cam_temp}])
        # df = pd.DataFrame([{'1.IMU_yaw(deg)': round(math.degrees(self.IMU_yaw),4), '2.IMU_diff (deg)': round(math.degrees(self.IMU_yaw_diff),4), '3.Max Degree(deg)': max_degree, '4.Mag dy(px)': dy}])
            ...
            try:
                if self.get_new_IMG_msg and self.get_new_imu_msg:
                    ...
                    if self.preproc_ct != 0 :
                        ...
                        max_dy, max_degree = self.mv_Calculator(prev_inliers, curr_inliers)
                        self.euler2dist(max_dy, max_degree)
                    ...
                    print("Processing Time : {%0.3f}"%(terminate_t - start_t))
